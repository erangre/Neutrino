...
Shell::Shell(neutrino *nparent) : nGenericPan(nparent)
{
    ...
    if (!qApp->property("pythonInstantiated").isValid()) {
...
//        PythonQt_init_QtBindings();
        ...
        QPointer<QMenu> menuPython = nPluginLoader::getMenu("Scripts",nparent);

    //        neu->my_w->toolBar->addAction(QIcon(":/icons/python.png"),"Python");

        nparent->my_w->menubar->addMenu(menuPython);
        settings.beginGroup("Shell");
        ...
        if (scriptdir.exists()) {
            ...
            foreach (QString sname, scriptlist) {
                QAction *action = new QAction(nparent);
        ...
        qApp->setProperty("pythonInstantiated",true);
    }
